...
import com.bank.details.TransactionDetails;
import com.bank.details.TransferDetails;
...
import com.bank.service.SaccountService;
import com.bank.service.TransferHistoryService;
...
public class AccountController {
	...
	private final String ifsc="BANK2023";
	...
	public TransferResponse transfer(@RequestBody TransferDetails details) {
		try {
			if(details.getIfsc().equals(ifsc)) 
			{
				...
				TransferResponse response=new TransferResponse();
						response.setSaccount(details.getSaccount());
						response.setResponseMessage("SWIFT code is incorrect");
						response.setTransferStatus(false);
						return response;
			}
		} catch (Exception e) {
			TransferResponse response=new TransferResponse();
			response.setSaccount(details.getSaccount());
			response.setResponseMessage("Please provide an SWIFT code");
			response.setTransferStatus(false);
			return response;
